...
import ActivityFilterSection from "./BranchFilteredSection";
...
export default function BranchIndex(props) {
  const suhbeader = useSubheader();
  suhbeader.setTitle("Asset List by Branch/Employee Report");
  ...
  useEffect(() => {
    if (props.location.state) {
      let auditObj = {
        ...
        locationObject: null,
        allocationObject: [],
        description: "Asset List by Branch/Employee Report page",
        requestObject: null,
      };
      ...
      .then((arr) => {
        ...
        setFormApiValues((prev) => {
          return {
            ...
            location: arr[1],
            users: arr[2],
            assets: [...new Set(arr[3].map((one) => one.asset))],
          };
        });
      });
  ...
  function initialFetch(string) {
    ...
    fetch(
      `${server_path}api/ActivityReport/GetAssetByBranchOrEmployee?FromDate=${formData.fromDate}&ToDate=${formData.toDate}&CategoryID=${formData.category}&AssetID=${formData.asset}&LocationID=${formData.location}&UserID=${formData.user}`,
