...
export default function MakeTable({
  ...
  const [multiIssueOpen, setMultiIssueOpen] = useState(false);
  ...
  function getExportFileBlob({ columns, data, fileType, fileName }) {
    if (fileType === "xlsx") {
      ...
      XLSX.utils.book_append_sheet(wb, ws1, "Asset Issue Data");
            ...
            <BottomWrapper>
              <BottomArea>
                {selectedFlatRows.length === 0 ? (
                  <IssueBtn
                    ...
                    onClick={() => {
                      setOpenSnack({
                        ...openSnack,
                        openSnackOpen: true,
                        message: "Please select at least one row to issue",
                        title: "fill all detail",
                      });
                  ...
                  <Link
                    to={{
                      pathname: "asset-issue/multi-issue",
                      state: () => {
                        let arr = [];
                        selectedFlatRows.forEach((one) => {
                          arr.push({
                            id: one.original.assetDetail_ID,
                            asset_Name: one.original.asset_Name,
                            asset_Tag: one.original.asset_Tag,
                            category: one.original.category,
                            location: one.original.location,
                          });
                        });
                        return arr;
                      },
                  ...
                  >
                    <IssueBtn variant="contained" color="secondary">
                      Issue
                    </IssueBtn>
                  </Link>
                )}
